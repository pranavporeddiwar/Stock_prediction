...
import pyotp
import re
from dotenv import load_dotenv  # CRITICAL: Reads your .env file
...
class DataFetcher:
    def __init__(self):
        try:
            # Path handling for your D: drive setup
            ...
            if os.path.exists(csv_path):
                ...
                print(f"⚠️ DataFetcher: {csv_path} not found. Using empty mapping.")
                self.symbols_lut = pd.DataFrame()
        except Exception as e:
            print(f"❌ DataFetcher: CSV Error: {e}")
            self.symbols_lut = pd.DataFrame()

        # Initialize the session after loading env variables
        self.api = self._init_session()

    def _init_session(self):
        """Generates the authenticated session for Angel One."""
        try:
            # Fetching from .env
            ...
            # Debug check: ensure the variables are actually loaded
    ...
    def get_enriched_data(self, symbol_input, mode="intraday"):
        ...
        # Priority: Search for Exact Symbol (e.g., TCS-EQ)
        ...
        try:
            ...
            if res.get('status') and res.get('data'):
                ...
                # Technical Indicators for LSTM Model
            ...
            print(f"⚠️ API Error: {res.get('message', 'Empty Data Received')}")
            return None
